...
- **On-brand**: Warm, scientific, empowering - the Zenith voice',
    ...
    // Add max_participants to cohorts if missing
    try {
        $db->exec("ALTER TABLE cohorts ADD COLUMN max_participants INTEGER DEFAULT NULL");
        echo "Added max_participants column to cohorts.\n";
    } catch (PDOException $e) {
    }

    // Multi-Currency: cohort_prices table for per-currency pricing
    try {
        $db->exec("
        ...
        ");
        echo "CohortPrices table created.\n";
    } catch (PDOException $e) {
    }

    // Multi-Currency: exchange_rates table with rate history
    try {
        $db->exec("
        ...
        ");
        echo "ExchangeRates table created.\n";
    } catch (PDOException $e) {
    }

    // Multi-Currency: Add display_currency to user_stats for user preference
    try {
        $db->exec("ALTER TABLE user_stats ADD COLUMN display_currency VARCHAR(3) DEFAULT 'USD'");
        echo "Added display_currency column to user_stats.\n";
    } catch (PDOException $e) {
    }
