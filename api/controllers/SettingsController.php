...
class SettingsController
{
    ...
    public function handleRequest($action, $key = null)
    {
        ...
        // FX rate endpoints
        if ($action === 'fx-rates' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            $this->getFXRates();
            return;
        }
        if ($action === 'fx-rates' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            $this->updateFXRate();
            return;
        }
        if ($action === 'cohort-prices' && $_SERVER['REQUEST_METHOD'] === 'GET') {
            $this->getCohortPrices();
            return;
        }
        if ($action === 'cohort-prices' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            $this->updateCohortPrice();
            return;
        }
        if ($action === 'refresh-fx-rates' && $_SERVER['REQUEST_METHOD'] === 'POST') {
            $this->refreshFXRates();
            return;
        }
    ...
    private function getFXRates()
    {
        $fxService = new FXRateService($this->db);
        $rates = $fxService->getAllRates();
        echo json_encode(['success' => true, 'rates' => $rates, 'base' => 'USD']);
    }

    private function updateFXRate()
    {
        if (!$this->verifyAdmin()) {
            http_response_code(403);
            echo json_encode(["message" => "Admin access required"]);
    ...
    private function getCohortPrices()
    {
        ...
        $fxService = new FXRateService($this->db);
        $prices = $fxService->getAllCohortPrices($cohortId);
        echo json_encode(['success' => true, 'prices' => $prices]);
    }

    private function updateCohortPrice()
    {
        if (!$this->verifyAdmin()) {
            http_response_code(403);
            echo json_encode(["message" => "Admin access required"]);
        ...
        if (!$cohortId || !$currency || $amount === null) {
            http_response_code(400);
            echo json_encode(["message" => "cohort_id, currency, and amount are required"]);
            return;
        }
        ...
        if ($stmt->execute([$id, $cohortId, $currency, $amount, $amount])) {
